...
//																  propertyDefinition.length() - name.length());
			...
			// vocabulary may have a hash namespace and use the underscore character as delimiter
			// example: http://localhost:8080/oslc4jmagicdraw/services/sysml-rdfvocabulary#NamedElement_name
			if(propertyDefinition.endsWith("_" + name)){
				namespace = propertyDefinition.substring(0,
						  propertyDefinition.length() - (name.length() + 1)) + "/";
			}
			// else voacublary may use a slash namespace
			// example: http://www.omg.org/sysml/NamedElement/name
			else{
				namespace = propertyDefinition.substring(0,
						  propertyDefinition.length() - name.length());
			}
